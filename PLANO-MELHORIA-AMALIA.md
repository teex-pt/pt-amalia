...
- **Routing de teachers por categoria** (Apache 2.0 todos; decidir por yield medido nos verificadores, não por fé):
  ...
  | Volume/velocidade (Fase 1) | **Gemma-4-26B-A4B-it** (MoE, ~70 tok/s no M5 Pro) | ✅ compatível com Apache 2.0: o paper PROPOR do consórcio (§3.3, Tabela 1) gerou dados com Gemma 3-27B («the best available open model for European Portuguese») e publica essas fatias como **Apache 2.0 / Synth. Gen.** — os outputs Gemma não têm direitos reclamados pela Google; só os pesos são Gemma ToU. Proveniência por amostra mantém tudo auditável. Candidato também para variedade/prosa; decidir por yield |
...
- **Fontes reais (complemento ao sintético):**
  ...
  - **Conteúdo legal — plataformas verificadas (2026-07-04):** textos oficiais isentos de direito de autor (CDADC); descontaminar contra `LegalBenchPT`; anonimizar decisões judiciais (RGPD).
    - *Pronto a usar:* `stjiris/portuguese-legal-sentences` no HF (**Apache 2.0**, acórdãos do STJ, projeto IRIS) — registo legal imediato, zero extração.
    - *Diário da República completo:* sem API bulk oficial (dados.gov.pt não tem; site oficial é OutSystems). Rotas: enumeração dos PDFs por data em diariodarepublica.pt (décadas recentes born-digital, sem OCR) + **API aberta do arquivo.pt** para o histórico — a rota do próprio consórcio (`arquivo_processing`). Crawl com rate-limit respeitoso.
    - *Bónus:* **EUR-Lex/CELLAR** — todo o direito da UE em pt-PT com paralelo EN, bulk verdadeiramente aberto (registo legal + pares de tradução).
    - 💎 Cada diploma do DR tem **sumário oficial** → dataset de sumarização com alvos gold por construção (`amalia-sum-dre`), na mesma filosofia verificável do resto do pipeline.
  - **BASE contratos públicos (base.gov.pt):** registo contratual/administrativo pt-PT, dados abertos.
  - **Teses universitárias (RCAAP — agrega IST, U.Porto, Coimbra, NOVA, etc.):** registo académico/técnico pt-PT nativo, ausente da mistura original. v1 só com metadados OAI-PMH (título + resumo + áreas + direitos — sem PDFs/OCR). **Filtrar por `dc:rights`: apenas CC-BY/CC0** (acesso aberto ≠ licença aberta; CC-BY-NC inviabiliza uso comercial). Usos: `amalia-qa-teses` (QA ancorado em resumos, verificação extrativa) e `amalia-par-teses` (resumos bilingues pt/en = corpus paralelo académico para a fatia de proteção de tradução). Sem risco de contaminação de benchmarks.
